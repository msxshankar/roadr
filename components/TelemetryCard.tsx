...
export default function TelemetryCard({
  ...
  return (
    <div className="theme-scope theme-panel liquid-glass rounded-2xl border border-white/10 p-4 text-gray-100 shadow-2xl animate-fade-in space-y-4 sm:p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          <span className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-[9px] font-mono uppercase text-gray-400">{provider}</span>
          <button type="button" onClick={() => setShowAlternatives((visible) => !visible)} disabled={alternatives.length === 0} aria-expanded={showAlternatives} className="inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-[9px] font-semibold text-gray-300 transition-colors hover:border-cyan-400/40 hover:text-cyan-200 disabled:cursor-not-allowed disabled:opacity-45" title={alternatives.length > 0 ? 'Compare up to three alternative routes' : 'No alternative routes were returned for this journey'}><GitBranch className="h-3 w-3" /> Alternative routes{alternatives.length > 0 && <span className="rounded-full bg-cyan-400/15 px-1 text-cyan-200">{Math.min(alternatives.length, 3)}</span>}</button>
        </div>
      </div>
      ...
      {onStartPreview && <div className="theme-section flex items-center justify-between gap-3 rounded-xl border border-white/10 p-3">
        <div className="min-w-0"><p className="text-[10px] font-mono uppercase tracking-wider text-gray-500">3D route preview</p><p className="mt-0.5 truncate text-xs font-semibold text-gray-200">Touch-turn camera with road lock</p></div>
        <button type="button" onClick={onStartPreview} aria-label="Start 3D drive preview" className="theme-primary-button inline-flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-[10px] font-extrabold tracking-wide transition-all hover:brightness-110 active:scale-[.98]"><Video className="h-3.5 w-3.5" /><span>Start 3D drive preview</span></button>
      </div>}
        ...
        {details.hasElevationData && details.elevationProfile.length > 1 && (
          <div className="rounded-xl border border-white/10 bg-black/25 p-2.5">
            ...
            <div className="flex h-12 items-end gap-px">
              {details.elevationProfile.map((sample, index) => <span key={`${sample.distanceMeters}-${index}`} title={`${sample.elevationM}m · ${sample.gradientPercent}%`} className="flex-1 rounded-t bg-teal-600" style={{ height: `${Math.max(8, ((sample.elevationM - minProfile) / profileRange) * 100)}%` }} />)}
            </div>
          </div>
        )}
      ...
      <div className="theme-section space-y-3 rounded-xl border border-cyan-500/25 bg-cyan-950/30 p-3.5">
        <div className="flex items-center justify-between"><div className="flex items-center space-x-2"><Fuel className="h-4 w-4 text-cyan-400" /><span className="text-xs font-semibold uppercase tracking-wider text-cyan-200">Fuel & cost estimate</span></div><div className="flex items-center space-x-1 rounded-full border border-emerald-500/30 bg-emerald-950/60 px-2 py-0.5 text-[10px] text-emerald-400"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" /><span className="font-mono">{isLiveFuelFetching ? 'Loading price' : `${liveFuelSource}: ${liveFuelPricePence}p/L`}</span></div></div>
        ...
        <div className="space-y-3 pt-1">
          <div className="space-y-1"><div className="flex justify-between text-xs font-mono"><span className="flex items-center gap-1.5 text-gray-300"><SlidersHorizontal className="h-3 w-3" />{vehicle ? `${vehicle.nickname} MPG` : 'Vehicle MPG'}</span><span className="font-bold text-gray-200">{mpg} MPG</span></div><input aria-label={`${vehicle?.nickname || 'Vehicle'} MPG`} type="range" min={15} max={120} step={1} value={mpg} onChange={(event) => onChangeMpg(parseInt(event.target.value, 10))} className="h-1.5 w-full cursor-pointer appearance-none rounded-lg bg-gray-800 accent-cyan-400" /></div>
      ...
      <button type="button" onClick={onRecordRoute} className="theme-warm-button flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold transition-all hover:brightness-110"><RouteIcon className="h-4 w-4" /> Record route to car</button>
    </div>
  );
}
