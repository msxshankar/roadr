            ...
            <table className="w-full min-w-[600px] border-collapse text-left text-sm">
              ...
              <tbody>
                {users.map((user) => <tr key={user.id} className="border-b border-white/5 last:border-0">
                  <th scope="row" className="px-4 py-3.5 font-semibold text-white sm:px-5"><span className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/10 text-[10px] font-bold text-cyan-200">{user.username.slice(0, 1).toUpperCase()}</span>{user.username}{user.id === currentUser.id && <span className="rounded-full bg-cyan-500/10 px-2 py-0.5 text-[9px] font-bold uppercase text-cyan-200">You</span>}</span></th>
